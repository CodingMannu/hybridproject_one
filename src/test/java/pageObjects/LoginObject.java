...
public class LoginObject extends TestBase {
	...
	public LoginObject(WebDriver driver) {
		...
		handleToastMessage = new HandleToastMessage(driver); // add this for handle from handle toast message
	}
...
//		LoginObject.closeDriver();
	...
	public void user_is_logged_in(String phoneNumber) {
		...
		if (isElementVisible(user_enters_valid_mobile_number, 5)) {
			logger.info("Mobile number input field is visible.");
			waitForElementToBeClickable(user_enters_valid_mobile_number, 3).sendKeys(userPhoneNumber);
			logger.info("Entered valid mobile number: " + userPhoneNumber);

			executionDelay(2);
			waitForElementToBeClickable(user_clicks_on_send_otp_button, 3).click();
			logger.info("Clicked on Send OTP button.");

			waitForElementToBeClickable(user_enters_valid_otp, 10).sendKeys(validOTP);

			logger.info("Entered valid OTP: " + validOTP);
			executionDelay(2);
			waitForElementToBeClickable(user_clicks_on_verify_OTP_button, 20).click();
			logger.info("Clicked on Verify OTP button.");

			if (isElementVisible(user_should_nevigate_to_the_home_page_of_the_website, 20)) {
				logger.info("User logged in successfully with mobile number: " + userPhoneNumber);
				...
				logger.error("Login failed for mobile number: " + userPhoneNumber);
			}
		} else {
			logger.error("Mobile number input field not visible. Login flow skipped." + userPhoneNumber);
		}
	}
	...
	public void validate_toast_message_by_key_and_type(String toastKey, String typeKey) {
		handleToastMessage.validateToastMessageByKeyAndType(toastKey, typeKey);
	}

	public void user_clicks_on_close_toast_message() {
		handleToastMessage.closeToastMessage();
	}
